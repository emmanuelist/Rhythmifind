// Import necessary modules and components from Radix UI and React Icons
import * as Dialog from "@radix-ui/react-dialog";
import { IoMdClose } from "react-icons/io";

// Define the interface for the Modal component's properties
interface ModalProps {
  isOpen: boolean;
  onChange: (open: boolean) => void;
  title: string;
  description: string;
  children: React.ReactNode;
}

// Implement the Modal component, which creates a customizable dialog box
const Modal: React.FC<ModalProps> = ({
  isOpen,
  onChange,
  title,
  description,
  children,
}) => {
  // Render the dialog box with the given properties and children
  return (
    <Dialog.Root open={isOpen} defaultOpen={isOpen} onOpenChange={onChange}>
      <Dialog.Portal>
        {/* Overlay for the dialog box */}
        <Dialog.Overlay className="bg-neutral-900/90 backdrop-blur-sm fixed inset-0" />

        {/* Content of the dialog box */}
        <Dialog.Content className="fixed drop-shadow-md border border-neutral-700 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90vw] md:max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none">
          {/* Title of the dialog box */}
          <Dialog.Title className="text-xl text-center font-bold mb-4">
            {title}
          </Dialog.Title>

          {/* Description of the dialog box */}
          <Dialog.Description className="mb-5 text-sm leading-normal text-center">
            {description}
          </Dialog.Description>

          {/* Children of the dialog box (custom content) */}
          <div>{children}</div>

          {/* Close button for the dialog box */}
          <Dialog.Close asChild>
            <button className="text-neutral-400 hover:text-white absolute top-[10px] right-[10px] inline-flex h-[25px] w-[25px] appearance-none items-center justify-center rounded-full focus:outline-none">
              <IoMdClose size={24} />
            </button>
          </Dialog.Close>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  );
};

// Export the Modal component for use in other modules
export default Modal;
